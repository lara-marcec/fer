...
class BTNode:
    ...
    def split(self)->BTNode:
        if self.size>=self.deg: # ako je velicina cvora nakon dodavanja vrijednosti veca ili jednaka stupnju stabla
            s=ceil(self.deg/2)-1 #broj vrijednosti na lijevoj strani nakon splittanja
            nl=BTNode(start=self.start,deg=self.deg,initsize=s) #novi cvor - lijeva strana
            nl.pn=self.pn
            v=self.start
            for i in range(s-1): v=v.next
            nm=v.next #onaj u sredini, s-ti?
            v.next=None
            v.setrightchild(nm.lc)
            nl.last=v
            nl._adjustParent()
            nr=self # desna strana
            nr.start,nr.size=nm.next,nr.size-1-s
            nm.next,nr.start.prev=None,None

            if nr.pn is not None: # 2b - The leaf node has a parent node
                nv=nr.pn.insert(nm.value) #srednja vrijednost cvora koji se splitta se stavlja u roditelj cvor 
                nv.setleftchild(nl) #lijeva strana srednje vrijednost  
                if nv.next is None:
                    nv.setrightchild(nv.prev.rc) #desna strana srednje vrijednosti
                    nv.prev.setrightchild(None)
                if nr.pn is not None: return nr.pn.split()

            else: # 2a - The leaf node has no parent node, meaning that the leaf node is the root node
                nroot=BTNode(value=nm.value,deg=self.deg,initsize=1) # novi cvor korijen
    ...
    def remove(self,v:BTValue)->BTNode:
        if v.lc is not None: #ako nije list, trazimo successora ili predecessora
            # the predecessor is always the rightmost value of the removed value left subtree,
			# the successor is always the leftmost value of the removed value right subtree
            (n,pv)=v.lc._maxvalue() # predecessor
            v.value=pv.value
            n._removevalue(pv)
            return n
        else: #ako je list
...
class BTree:
    ...
    def _insert(self,val)->None:
        (n,v)=self.root.search(val) # trazi vrijednost koja se dodaje, vraca cvor koji je u range te vrijednosti, cvor koji je zadnji pretrazivan
        n.insert(val) #vrijednost se dodaje u taj cvor
        nroot=n.split() #dijeli cvor
