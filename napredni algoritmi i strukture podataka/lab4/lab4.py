...
class IntervalTree:
    ...
    def _query(self, node: IntervalNode, qx1: float, qx2: float, qy1: float, qy2: float) -> list:
        ...
        if is_in_range(qx1, qx2, val):
            for ivs in node.tree_right:
                if is_in_window(ivs['point'], qx1, None, qy1, qy2):
                    results.add(ivs['interval'])

            for ivs in node.tree_left:
                if is_in_window(ivs['point'], None, qx2, qy1, qy2):
                    results.add(ivs['interval'])

            if(node.left_child is not None ):
                results = results.union(self._query(node.left_child, qx1, qx2, qy1, qy2))
            if(node.right_child is not None):
                results = results.union(self._query(node.right_child, qx1, qx2, qy1, qy2))
