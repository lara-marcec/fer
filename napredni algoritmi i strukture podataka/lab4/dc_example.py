    ...
    def _query(self, node: IntervalNode, qx1: float, qx2: float, qy1: float, qy2: float) -> list:
        ...
        if is_in_range(qx1, qx2, node.x_median):
            for interval in node.intervals:
                if is_in_window(interval[0], qx1, qx2, qy1, qy2):
                    results.append(interval)

            results += self._query(node.left_child, qx1, qx2, qy1, qy2)
            results += self._query(node.right_child, qx1, qx2, qy1, qy2)
        
        
        elif node.x_median < qx1:
            for interval_dict in node.tree_right:
                if is_in_window(interval_dict['point'], qx1, qx2, qy1, qy2):
                    results.append(interval_dict['interval'])

            results += self._query(node.right_child, qx1, qx2, qy1, qy2)

        elif node.x_median > qx2:
            for interval_dict in node.tree_left:
                if is_in_window(interval_dict['point'], qx1, qx2, qy1, qy2):
                    results.append(interval_dict['interval'])
