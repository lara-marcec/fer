...
class SkipList:
    ...
    def __init__(self, n: int = 10000, p: float = 0.5):
        ...
        self.p = p
        self.n = n
        self.max_level = floor( 1 + math.log(n, 1/p) )
    ...
    def insert(self, value: int) -> None:
        ...
        if succ_node:
            succ_node.predecessor = new_node

        for lvl in range(1, level):
            while pred_node is not self.head and len(pred_node.pointers) <= lvl:
                pred_node = pred_node.predecessor
            if pred_node is self.head and len(pred_node.pointers) <= lvl:
                self.head.pointers[lvl] = new_node
            else:
                new_node.pointers[lvl] = pred_node.pointers[lvl]
                pred_node.pointers[lvl] = new_node
